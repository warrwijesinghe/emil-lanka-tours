...
export function getRoundTourDayImage(day: { title: string; route: string; experiences: string[] }) {
  const text = `${day.title} ${day.route} ${day.experiences.join(' ')}`.toLowerCase();
...
export function getRoundTourDayImages(day: { title: string; route: string; experiences: string[] }) {
  const text = `${day.title} ${day.route} ${day.experiences.join(' ')}`.toLowerCase();
