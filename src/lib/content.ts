import { z } from "zod";
import { dayTours as dayTourFiles } from "@/data/day-tour-files";
import destinationsJson from "@/data/destinations.json";
import { tourDestinations } from "@/data/tour-destinations";
import testimonialsJson from "@/data/testimonials.json";
import type { DayTour, Destination } from "@/types/content";

const seo = z.object({
  title: z.string().min(1),
  description: z.string().min(1),
});
const destinationSchema = z.object({
  id: z.string(),
  slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/),
  name: z.string().min(1),
  region: z.string().min(1),
  shortDescription: z.string().min(1),
  description: z.string().min(1),
  mainImageUrl: z.string().startsWith("/images/"),
  imageAlt: z.string().min(1),
  highlights: z.array(z.string()),
  bestFor: z.array(z.string()),
  suggestedVisitDuration: z.string().min(1),
  seo,
  featured: z.boolean(),
});
function uniqueSlugs(items: { slug: string }[], label: string) {
  if (new Set(items.map((x) => x.slug)).size !== items.length)
    throw new Error(`Duplicate ${label} slug in local JSON.`);
  return items;
}
export const dayTours = uniqueSlugs(dayTourFiles, "day tour") as DayTour[];
export const visibleDayTours = dayTours;
export const destinations = uniqueSlugs(
  [...z.array(destinationSchema).parse(destinationsJson), ...tourDestinations],
  "destination",
) as Destination[];
export const testimonials = z
  .array(
    z.object({
      name: z.string(),
      quote: z.string(),
      location: z.string().optional(),
    }),
  )
  .parse(testimonialsJson);
export const stripHtml = (value: string) =>
  value
    .replace(/<[^>]*>/g, " ")
    .replace(/\s+/g, " ")
    .trim();
export const getDayTour = (slug: string) =>
  dayTours.find((item) => item.slug === slug);
export const getVisibleDayTour = (slug: string) =>
  visibleDayTours.find((item) => item.slug === slug);
export const getDestination = (slug: string) =>
  destinations.find((item) => item.slug === slug);
